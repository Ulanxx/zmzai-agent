...
import { SessionRunner, createFrameworkSession, type RunnerDeps } from "@/framework/core/runtime/runner";
...
vi.mock("@/framework/core/events/bus", () => ({
  publishFrameworkEvent: vi.fn().mockImplementation(async (event: { sessionId: string; type: string; data: unknown }) => ({
    ...event,
    id: "evt_test",
    seq: 1,
...
// Lease stamping touches the real mongoose model — stub it for tests.
vi.mock("@/framework/core/session/mongo-models", () => ({
  FrameworkSessionModel: { updateOne: vi.fn().mockResolvedValue({}) },
...
const publishMock = vi.mocked(publishFrameworkEvent);
...
function publishedTypes(): string[] {
  return publishMock.mock.calls.map((call) => (call[0] as { type: string }).type);
}

function lastStatus(): string | null {
  const statuses = publishMock.mock.calls
    .filter((call) => (call[0] as { type: string }).type === "session.status")
...
describe("SessionRunner", () => {
  it("text-only run: emits full part chain and settles idle", async () => {
    const { runner, store } = makeHarness([fauxAssistantMessage("任务完成，已读取 1 个文件。")]);
    const session = await makeSession(store);

    await runner.prompt(session.id, { text: "读取代码" });
    await waitFor(() => publishedTypes().filter((type) => type === "session.status").length >= 2);

    const types = publishedTypes();
  ...
  it("bash run: permission.asked → once reply → sandbox executes → tool part completes", async () => {
    const { runner, store, toolContext } = makeHarness([
    ...
    await waitFor(() => publishedTypes().includes("permission.asked"));
    const asked = publishMock.mock.calls.find((call) => (call[0] as { type: string }).type === "permission.asked");
    const request = (asked![0] as { data: { request: { id: string; patterns: string[] } } }).data.request;
    expect(request.patterns).toEqual(["npm run build"]);

    const replied = await runner.replyPermission(session.id, request.id, "once");
    expect(replied).toBe(true);

    await waitFor(() => lastStatus() === "idle");
  ...
  it("bash run: reject blocks execution and the model sees the reason", async () => {
    ...
    await waitFor(() => publishedTypes().includes("permission.asked"));
    const asked = publishMock.mock.calls.find((call) => (call[0] as { type: string }).type === "permission.asked");
    const request = (asked![0] as { data: { request: { id: string } } }).data.request;
  ...
  it("queues a prompt submitted mid-run and drains it after settlement", async () => {
    const { runner, store, faux } = makeHarness([fauxAssistantMessage([fauxToolCall("bash", { program: "sleep", args: ["1"] })])]);
    const session = await makeSession(store);
    ...
    const asked = publishMock.mock.calls.find((call) => (call[0] as { type: string }).type === "permission.asked");
    await runner.replyPermission(session.id, (asked![0] as { data: { request: { id: string } } }).data.request.id, "once");
