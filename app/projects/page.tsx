...
export default function ProjectsPage() {
  ...
  return <main className="product-page">
    ...
    <section className="project-list">{items.length ? items.map(({ project, tasks }) => <article className="project-row" key={project.projectId}><div className="project-row-main"><div className="project-row-title"><span className="project-mark"><Icon name="folder" size={15} /></span><div><h2>{project.name}</h2><p>{project.description || "尚未添加项目描述"}</p></div></div><span className="project-workspace">{workspaces.find((workspace) => workspace.id === project.workspaceId)?.name ?? "Workspace"}</span></div><div className="project-row-tasks">{tasks.length ? tasks.slice(0, 4).map((task) => <Link href={`/fw/t/${task.taskId}`} key={task.taskId}><span data-status={task.status} />{task.title || "未命名任务"}</Link>) : <span className="project-empty">还没有归属任务</span>}<IconButton size="sm" label="打开项目"><Icon name="arrow-right" size={13} /></IconButton></div></article>) : <div className="product-empty"><Icon name="folder" size={22} /><strong>还没有项目</strong><p>创建一个项目，给长期任务保留上下文。</p></div>}</section>
  </main>;
}
