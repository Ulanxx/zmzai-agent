...
export async function POST(_: Request, context: { params: Promise<{ workspaceId: string; connectorId: string }> }) {
  ...
  const connector = await testWorkspaceConnector({ userId: user.id, workspaceId, connectorId });
  if (!connector) return apiError("CONNECTOR_NOT_FOUND", 404, "连接器不存在或无权访问");
