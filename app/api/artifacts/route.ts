...
export async function GET(request: NextRequest) {
  ...
  const records = await SandboxArtifactModel.find({ userId: user.id, ...(tag ? { tags: tag } : {}) }).sort({ createdAt: -1 }).limit(limit).lean();
  ...
  }).filter((artifact) => !projectId || artifact.projectId === projectId) }, { headers: { "cache-control": "no-store" } });
}
